    ...
    class BP_Better_Messages_Functions
    {
        ...
        public function find_existing_threads($from, $to){
            ...
            $query_from = $wpdb->prepare("SELECT
                ...
                AND {$bp_prefix}bp_messages_recipients.is_deleted = 0", $from);

            $query_to = $wpdb->prepare("SELECT
                  {$bp_prefix}bp_messages_recipients.thread_id
                FROM {$bp_prefix}bp_messages_recipients
                WHERE {$bp_prefix}bp_messages_recipients.user_id = %d
                AND {$bp_prefix}bp_messages_recipients.is_deleted = 0", $to);
            ...
            if( count( $threads_between_users ) > 0 ) {
                ...
                FROM {$bp_prefix}bp_messages_recipients
                WHERE {$bp_prefix}bp_messages_recipients.thread_id IN {$threads_in}
                GROUP BY thread_id
                HAVING count = 2";
        ...
        public function render_thread( $thread, $user_id = false )
        {
            ...
            ?><div class="thread <?php echo implode(' ', $classes); ?>"
                ...
                <div class="time">
                    <?php if( $delete_allowed ){ ?><span class="delete" data-nonce="<?php echo wp_create_nonce( 'delete_' . $thread->thread_id ); ?>"><i class="fas fa-times" aria-hidden="true"></i></span><?php } ?>
