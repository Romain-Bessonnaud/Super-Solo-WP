...
(function ($) {
    ...
    var statusColorRead = BP_Messages['bpbm_color'];
    var statusColorUnread = '#898989';
    ...
    jQuery(document).on('bp-better-messages-update-unread', function( event, unread ) {
        ...
        if( jQuery('body').hasClass('my-account') ) {
            var tab = jQuery('#user-bp_better_messages_tab');
            if (tab.length > 0) {
                ...
                if (unread > 0) {
                    if (count.length > 0) {
                        ...
                        jQuery('<span class="count">' + unread + '</span>').appendTo(tab);
                    }
                } else {
                    count.remove();
                }
    ...
    function refreshSite() {
        ...
        $.post(BP_Messages.ajaxUrl, {
            ...
            BP_Messages.total_unread = response.total_unread;
    ...
    function renderMessage(message, selector) {
        ...
        if(message.user_id == BP_Messages['user_id']) {
            messageHtml += '<span class="status" title="' + BP_Messages['strings']['sent'] + '"></span>';
        }
        messageHtml += '<span class="message-content">' + message.message + '</span></li>';
    ...
    function showMessage(thread_id, message, name, avatar) {
        ...
        var findVisibleThread = $('.bp-messages-wrap .threads-list .thread[data-id="' + thread_id + '"]:visible');
